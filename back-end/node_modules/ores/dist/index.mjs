...
function resolveModuleURL(id, options = {}) {
  if (typeof id !== "string") {
  ...
  const urls = _normalizeResolveParents(options.from);
  let resolved;
...
function _normalizeResolveParents(inputs) {
  const urls = (Array.isArray(inputs) ? inputs : [inputs]).flatMap(
    (input) => _normalizeResolveParent(input)
  );
  if (urls.length === 0) {
...
function _normalizeResolveParent(input) {
  if (!input) {
    return [];
  }
...
export { resolveModulePath, resolveModuleURL };
